...
#TODO: Fix the spinning when spawning from multiple sides.
...
@export var spawn_count: int = 1                        ## Number of objects to spawn per cycle per direction if throwing in multiple directions
...
@export var throw_direction: Vector2 = Vector2.LEFT     ## Direction in which objects will be thrown if not random
@export var throw_multiple_directions: bool = false     ## Toggle to allow throwing in multiple directions per spawn
#NOTE: Know issue with the custom throw directions bellow. One side spawns properly but the other starts spinning. 
@export var throw_directions: Array[Vector2] = [Vector2.LEFT, Vector2.RIGHT] ## Directions for multi-throw. Add more directions as needed.
...
func spawn_object():
	...
	for direction in directions_to_use:
		for _i in range(spawn_count):  # Spawn as many objects as specified for each direction
			...
			if random_position:
				# Generate a random position within a specified radius from spawner's center
			...
			if should_throw:
				if object_instance is PhysicsBody2D:  # Check if the object can react to physics
					...
					object_instance.apply_torque_impulse(randf_range(0, 0)) # Random torque for variety. Change values or completely remove if not needed.
				else:
			...
			if max_instances != -1 and objects_spawned >= max_instances:
				...
				return  # Stop the function if max instances are reached
